...
2. **Commit hygiene and base freshness**. Is the PR a clean set of
   ...
   branch based off a recent `master`, or has the base diverged far
...
7. **New dependency surface**. Any new crates, system deps,
   build-time requirements, or external-service dependencies? A new
   dependency of `tollgate-protocol` or `tollgate-core` must build
   `no_std`. A git dependency must be pinned to a tag or rev, and cdk
   crates must all share one version with what `cdk-spilman` uses.
8. **New test coverage**. Are the new code paths covered, are the
   tests scoped correctly (unit / integration / end-to-end), and
   are there obvious test gaps? Behavior between two nodes needs a
   topology under `testing/`, not only a unit test. Don't reflag
   anything CI already enforces (formatting, lint, the `no_std`
   build, type errors, unit-test pass/fail).
9. **Documentation impact**. Does this need a CHANGELOG entry,
   rustdoc updates, design-doc changes
   ([docs/design/](docs/design/)), README adjustments, or changes to the
   example configs under [packaging/](packaging/) and
   [testing/](testing/)? A change that makes a `tollgate-core` test
   disagree with a design document's worked example has to say which
...
10. **Security vulnerabilities**. Any new attack surface,
    ...
    paths, secret-handling concerns, or side-channel exposure? Money
    moves here: look for a message reaching core before its signature
    is verified, a peer drawing more than its grant, a channel update
    accepted that lowers a balance, arithmetic that is not saturating,
    and wallet tokens or secret keys reaching a log.
...
```text
https://github.com/OpenTollGate/tollgate-rs/blob/<full-40-char-sha>/<path>#L<start>-L<end>
```
